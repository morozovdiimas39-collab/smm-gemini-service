...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if mode == 'process_section':
            ...
            # КРИТИЧНО: Ограничиваем до 600 слов, чтобы Gemini успел за 25 секунд
            target_words = min(target_words, 600)
            
            prompt = f"""Ты студент, который пишет {section['doc_type']} на тему: {section['subject']}
...
- Напиши ТОЛЬКО текст раздела без заголовка"""
            ...
            try:
                ...
                return {
                    'statusCode': 200,
                    'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                    'body': json.dumps({'message': 'Раздел сгенерирован', 'section_id': str(section['id'])}, ensure_ascii=False),
                    'isBase64Encoded': False
                }
            except Exception as e:
...
- Напиши ТОЛЬКО текст раздела без заголовка"""
        ...
        if mode == 'check_quality':
            ...
            try:
                scores = check_content_quality(text, api_key, proxy_url)
                ...
                return {
                    'statusCode': 500,
                    'headers': {'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'},
                    'body': json.dumps({'error': f'Ошибка проверки качества: {str(e)}'}),
                    'isBase64Encoded': False
                }
